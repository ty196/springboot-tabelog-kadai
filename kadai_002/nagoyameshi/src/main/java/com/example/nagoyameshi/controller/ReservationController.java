...
public class ReservationController {
	...
	public ReservationController(ReservationRepository reservationRepository, RestaurantRepository restaurantRepository, UserRepository userRepository,
			                     ReservationService reservationService, RegularHolidayRestaurantService regularHolidayRestaurantService) {
    ...
    public String index(@AuthenticationPrincipal UserDetailsImpl userDetailsImpl, @PageableDefault(page = 0, size = 15, sort = "id") Pageable pageable, Model model) {  
	...
	public String register(@AuthenticationPrincipal UserDetailsImpl userDetailsImpl,
		...
		if (roleName.equals("ROLE_FREE_MEMBER")){
		 ...
		 Restaurant restaurant = restaurantRepository.getReferenceById(restaurantId);    
	        ...
	        List<Integer> restaurantRegularHolidays = regularHolidayRestaurantService.getRestaurantRegularHolidays(restaurant);
    ...
    public String create(@AuthenticationPrincipal UserDetailsImpl userDetailsImpl,  
		...
		if (roleName.equals("ROLE_FREE_MEMBER")){
		 ...
		 if (reservationRegisterForm.getReservationDate() != null && reservationRegisterForm.getReservationTime() != null) {
	            LocalDateTime reservationDateTime = LocalDateTime.of(reservationRegisterForm.getReservationDate(), reservationRegisterForm.getReservationTime());
	            
	            if (!reservationService.isAtLeastTwoHoursInFuture(reservationDateTime)) {
	                FieldError fieldErrorTooSoon = new FieldError(bindingResult.getObjectName(), "reservationTime", "当日の予約は2時間前までにお願いいたします。");
	                bindingResult.addError(fieldErrorTooSoon);
	            }
	            
	            if (!reservationService.isTwoHoursApartFromOthers(reservationDateTime, user)) {
	                FieldError fieldErrorAlreadyReserved = new FieldError(bindingResult.getObjectName(), "reservationTime", "その時間帯はすでに予約済みです。同日に予約する場合、少なくとも2時間以上空ける必要があります。");
	            ...
	            if (reservationRegisterForm.getNumberOfPeople() != null && remainingSeats < reservationRegisterForm.getNumberOfPeople()) {
	                if (remainingSeats == 0) {
	                    FieldError fieldErrorFull = new FieldError(bindingResult.getObjectName(), "numberOfPeople", "その時間帯は満席です。");
	                    bindingResult.addError(fieldErrorFull);
	                } else {
	                    FieldError fieldErrorFewSeats = new FieldError(bindingResult.getObjectName(), "numberOfPeople", "その時間帯の残り予約可能人数は" + remainingSeats + "名です。");
